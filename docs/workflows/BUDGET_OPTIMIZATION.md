...
| 模型 | 适用场景 | 相对成本 |
| :--- | :--- | :--- |
| `manus-1.6-nano` | 极简单任务（文档更新、配置修改） | 最低 |
...
**原则**：除非任务明确需要更高级的推理能力，否则默认使用 `manus-1.6`。

## 任务分类与模型映射

### 使用 `manus-1.6-nano` 的场景

*   更新 README 或文档
*   修改配置文件（如 `vite.config.js`）
*   添加简单的依赖项到 `package.json`

**示例任务**：
*   VOI-60: 修复 Vite 构建错误（添加 terser 依赖）

### 使用 `manus-1.6-lite` 的场景
...
通过合理选择模型，我们可以在保证任务质量的前提下，显著降低项目的 API 调用成本。**默认使用 `manus-1.6`** 是当前最佳实践。
